...
struct MyStringHash {
    ...
    HASH_INDEX_T operator()(const std::string& k) const
    {
        ...
        unsigned long int initialVar, placeholder, checker; 
        ...
        if(size > 6)
        {
          ...
          while (placer < 5)
          {
            ...
            do
            {
              ...
              if(strTemp[j] != '!')
              {
                initialVar = letterDigitToNumber(strTemp[j]); 
              }
              total = (total * 36); 
              total += initialVar; 
              j = j + 1; 
        ...
        switch(firstSize)
        {
          ...
          do
          {
            initialVar = letterDigitToNumber(k[i]); 
            total = (total * 36) + initialVar;
            ++checker; 
            ++i; 
          }
          while (i <= 5);
        ...
        if(size < 6)
        {
          ...
          while (checks < 6)
          {
            ...
            if(strTemp[checks] != '!')
            {
              initialVar = letterDigitToNumber(k[checks]); 
            }
            total = ((total * 36)) + initialVar + user; 
            checks = checks + 1; 
          }
          checks = 1; 
          total *= rValues[4]; 
